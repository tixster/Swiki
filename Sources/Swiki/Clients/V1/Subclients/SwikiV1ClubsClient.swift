...
public extension SwikiV1ClubsClient {
    func index(query: SwikiQuery = [:]) async throws -> [SwikiClub] { try await list(query: query) }
    func get(id: String, query: SwikiQuery = [:]) async throws -> SwikiClub { try await resourceClient.get(id: id, query: query) }
    ...
    func ranobe(id: String, query: SwikiQuery = [:]) async throws -> [SwikiManga] {
        try await request(.get, id: id, action: "ranobe", query: query)
    }
    func characters(id: String, query: SwikiQuery = [:]) async throws -> [SwikiCharacter] {
        try await request(.get, id: id, action: "characters", query: query)
    }
    func members(id: String, query: SwikiQuery = [:]) async throws -> [SwikiUser] {
        try await request(.get, id: id, action: "members", query: query)
    }
    func images(id: String, query: SwikiQuery = [:]) async throws -> [SwikiClubImage] {
        try await request(.get, id: id, action: "images", query: query)
    }
    func join(id: String, query: SwikiQuery = [:]) async throws {
        try await request(.post, id: id, action: "join", query: query)
    }
    func leave(id: String, query: SwikiQuery = [:]) async throws {
