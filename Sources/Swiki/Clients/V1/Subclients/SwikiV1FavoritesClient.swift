...
public extension SwikiV1FavoritesClient {
    func create(linkedType: String, linkedId: String, query: some SwikiQueryConvertible = [:] as SwikiQuery) async throws -> SwikiFavorite {
        try await request(.post, id: linkedType, action: linkedId, query: query)
    }
    func delete(linkedType: String, linkedId: String, query: some SwikiQueryConvertible = [:] as SwikiQuery) async throws {
