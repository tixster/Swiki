...
public extension SwikiV1PeopleClient {
    func index(query: SwikiQuery = [:]) async throws -> [SwikiPerson] { try await list(query: query) }
    func get(id: String, query: SwikiQuery = [:]) async throws -> SwikiPerson { try await resourceClient.get(id: id, query: query) }
    func roles(id: String, query: SwikiQuery = [:]) async throws -> [[SwikiPersonRole]] {
        try await request(.get, id: id, action: "roles", query: query)
    }
    func works(id: String, query: SwikiQuery = [:]) async throws -> [SwikiPersonWork] {
