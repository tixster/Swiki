...
public extension SwikiV1AnimesClient {
    func get(query: some SwikiQueryConvertible = [:] as SwikiQuery) async throws -> [SwikiAnimeV1Preview] {
        try await request(.get, query: query)
    }
    func get(id: String, query: some SwikiQueryConvertible = [:] as SwikiQuery) async throws -> SwikiAnimeV1 { try await resourceClient.get(id: id, query: query) }
    ...
    func similar(id: String, query: some SwikiQueryConvertible = [:] as SwikiQuery) async throws -> [SwikiAnimeV1Preview] {
