...
struct SwikiGraphQLOperationSource: Sendable {
    let path: String
    let source: String
}

struct SwikiGeneratedSwiftFile: Sendable {
    let fileName: String
    let content: String
}
...
private func writeGeneratedOperations(
    ...
    for file in makeGeneratedOperationFiles(operations) {
        let outputPath = outputDirectoryURL
            .appendingPathComponent(file.fileName)
            .path
        try writeGeneratedFile(file.content, to: outputPath)
    }
}

private func makeGeneratedOperationFiles(_ operations: [RenderedOperation]) -> [SwikiGeneratedSwiftFile] {
    var files: [SwikiGeneratedSwiftFile] = []
    ...
    files.append(
        SwikiGeneratedSwiftFile(
            fileName: "SwikiGraphQLOperations.generated.swift",
            content: namespaceLines.joined(separator: "\n")
        )
    )
    ...
    for operation in operations.sorted(by: { $0.typeName < $1.typeName }) {
        ...
        files.append(
            SwikiGeneratedSwiftFile(
                fileName: "SwikiGraphQLOperations+\(operation.typeName).generated.swift",
                content: operationLines.joined(separator: "\n")
            )
        )
    }
...
private func renderedOperations(
    schemaSource: String,
    ...
    return try operationFiles.flatMap { operationFile in
        try generator.render(operationFile: operationFile)
    }
}

func generateSwikiGraphQLOperationFilesInMemory(
    schemaSource: String,
    operationSources: [SwikiGraphQLOperationSource]
) throws -> [SwikiGeneratedSwiftFile] {
    let parsedOperationFiles = try operationSources.map { operationSource in
        let document = try parse(source: operationSource.source)
        return OperationFile(
            path: operationSource.path,
            document: document,
            source: operationSource.source.trimmingCharacters(in: .whitespacesAndNewlines)
        )
    }
    .sorted(by: { $0.path < $1.path })

    let operations = try renderedOperations(
        schemaSource: schemaSource,
        operationFiles: parsedOperationFiles
    )
    return makeGeneratedOperationFiles(operations)
}

func runSwikiGraphQLOperationGenerator(
    schemaPath: String,
    operationsPath: String,
    outputPath: String
) throws {
    let schemaSource = try String(contentsOfFile: schemaPath, encoding: .utf8)
    let operationFiles = try readOperationFiles(from: operationsPath)
    let operations = try renderedOperations(schemaSource: schemaSource, operationFiles: operationFiles)
    ...
    try writeGeneratedOperations(operations, to: outputDirectory)
}
...
private struct SwikiGraphQLOperationGeneratorCommand: ParsableCommand {
    ...
    mutating func run() throws {
        try runSwikiGraphQLOperationGenerator(
            schemaPath: schema,
